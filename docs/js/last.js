...
const npmicon = '<svg viewBox="0 0 32 32" version="1.1" width="16" height="16" aria-hidden="true"><path d="M0 0v32h32v-32h-32zM26 26h-4v-16h-6v16h-10v-20h20v20z"></path></svg>'
...
const showRecent = function(selector, data) {
  ...
  for (var el in data) {
    ...
    divlist.innerHTML = "<h3>" + data[el].name + ' <span>' + data[el]["dist-tags"].latest + '</span></h3>';
    divlist.insertAdjacentHTML("beforeend", '<a href="https://www.npmjs.com/package/' + data[el].name + '" class="icon npm" target="_new">' + npmicon + '</a>');
    if (data[el].homepage) {
      divlist.insertAdjacentHTML("beforeend", '<a href="' + data[el].homepage + '" class="icon github" target="_new">' + githubicon + '</a>');
    }
...
const shownb = function(selector, legend, nb) {
  const div = document.querySelector("div#" + selector);
  div.insertAdjacentHTML("beforeend", nb + '<br><span>' + legend + '</span>');
}
...
fetch("data/all_packages.json").then(function(response) {
  ...
  shownb("packagesnb", "packages", packagesNb);
  ...
  shownb("contributors", "contributors", data.contributors);
